...
string decodeString(string s) {
  ...
  for (auto ch : s)  //^ learning auto use
  {
    if (ch == ']') {
      ...
      // NOTE isDigit --> take char tell weather digit or not -->here we have
      // string so s[0]
      while (!st.empty() && st.top() != "[") {
        string top = st.top();
        // stringToRepeat.push_back(top);//! string is not single char , can add
        // like this
      ...
      while (!st.empty() &&
             isdigit(st.top()[0]))  // L: [0] because stack kai string sai need
                                    //L: to pick 0 index element , isDigit check
      {
        numericTimes += st.top();
        st.pop();
      }
